...
const AutoResizeCaption = () => {
  ...
  useEffect(() => {
    ...
    const combinedText =
      postOverlay.type === "image_icon" || postOverlay.type === "image_gif"
    ...
    const baseWidth = getTextWidth(
      combinedText,
      (postOverlay.type === "image_icon" || postOverlay.type === "image_gif") ? imageIconRef : textareaRef
    );
    const padding = 32;
    ...
    setWidth(
      (postOverlay.type === "image_icon" || postOverlay.type === "image_gif") && finalWidth < defaultImageIconWidth
  ...
  const MusicOverlay = ({ postOverlay }) => {
    let parsedMusic = postOverlay.music
    if (typeof postOverlay.music === "string") {
    try {
      parsedMusic = JSON.parse(postOverlay.music);
    } catch (err) {
  ...
  return (
    <div ref={parentRef} className="relative w-full">
      {(postOverlay.type === "image_icon" || postOverlay.type === "image_gif") ? (
        ...
        <MusicOverlay postOverlay={postOverlay} />
      ) : (
        <textarea
          ref={textareaRef}
          value={
