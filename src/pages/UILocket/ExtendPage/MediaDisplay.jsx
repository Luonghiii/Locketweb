...
const MediaPreview = ({ loading, countdown, capturedMedia }) => {
  ...
  const toggleFlashlight = async () => {
    ...
    try {
      ...
      toast.error('Camera này không thể bật flashlight!');
    }
  };
    ...
    const startCamera = async () => {
      try {
        ...
        if (videoTrack) {
          ...
          // Reset flashlight state when camera changes
          setIsFlashlightOn(false);
          
          // Check torch support for current camera
          const hasTorch = capabilities.torch && 
            typeof capabilities.torch === 'boolean' && 
            capabilities.torch;
          console.log(`Torch support for ${cameraMode} camera:`, hasTorch);
          setHasFlashlight(hasTorch);
    ...
    <>
      ...
      >
        ...
        {cameraActive && !preview && !selectedFile && !capturedMedia && (
          ...
          className={`
            w-8 h-8 bg-gray-500 rounded-full flex items-center justify-center text-white 
            hover:bg-gray-500 transition-all duration-200 backdrop-blur-sm
          `}
          title="Bật flash"
        >
          {isFlashlightOn ? <IoFlashSharp size={20}/> : <IoFlashOutline size={20}/>}
        </button>
        </div>
        )}
        {(cameraActive || preview || selectedFile || capturedMedia) && (
